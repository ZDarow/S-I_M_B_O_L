...
def gen_disc_brake():
    ...
    # Brake disc (rotor)
    svg += '<circle cx="300" cy="200" r="100" fill="none" stroke="#666" stroke-width="8"/>'
    svg += '<circle cx="300" cy="200" r="70" fill="none" stroke="#666" stroke-width="3" stroke-dasharray="8,4"/>'
    svg += '<circle cx="300" cy="200" r="15" fill="#999" stroke="#666" stroke-width="2"/>'
    ...
    # Brake pads
    svg += '<rect x="220" y="175" width="30" height="50" rx="3" fill="#e65100" stroke="#bf360c" stroke-width="2"/>'
    svg += label("Колодка", 235, 168, size=10, color="#bf360c")
    svg += label("(внутренняя)", 235, 160, size=9, color="#bf360c")

    svg += '<rect x="350" y="175" width="30" height="50" rx="3" fill="#e65100" stroke="#bf360c" stroke-width="2"/>'
    svg += label("Колодка", 380, 168, size=10, color="#bf360c")
    svg += label("(наружная)", 380, 160, size=9, color="#bf360c")

    # Caliper
    svg += '<path d="M 210 165 L 390 165 L 390 240 L 210 240 Z" fill="none" stroke="#1565c0" stroke-width="3" rx="5"/>'
    svg += '<rect x="210" y="165" width="180" height="75" rx="5" fill="none" stroke="#1565c0" stroke-width="3"/>'
    svg += label("Суппорт", 300, 155, size=11, color="#1565c0", bold=True)

    # Piston
    svg += '<rect x="260" y="178" width="20" height="44" rx="10" fill="#bbdefb" stroke="#1565c0" stroke-width="2"/>'
    svg += label("Поршень", 240, 245, size=9, color="#1565c0")
    svg += arrow(270, 222, 250, 222)  # piston pushes pad

    # Brake line
    svg += '<path d="M 300 178 L 300 130 L 450 130" fill="none" stroke="#c62828" stroke-width="3"/>'
    svg += '<circle cx="450" cy="130" r="12" fill="#c62828"/>'
    ...
    # Legend
    svg += '<rect x="40" y="340" width="12" height="12" fill="#e65100" stroke="#bf360c" rx="2"/>'
    svg += label("Колодка (накладка + металл)", 60, 351, size=10, color="#333")
    svg += '<rect x="40" y="360" width="12" height="12" fill="none" stroke="#1565c0" rx="2" stroke-width="2"/>'
...
def gen_suspension():
    ...
    # Upper mount
    svg += '<rect x="200" y="30" width="100" height="25" rx="5" fill="#78909c" stroke="#546e7a" stroke-width="2"/>'
    svg += label("Верхняя опора\n(подушка стойки)", 250, 18, size=10, color="#546e7a")

    # Spring
    svg += '<path d="M 215 55 L 200 80 L 300 100 L 200 140 L 300 160 L 200 200 L 300 220 L 215 250" fill="none" stroke="#e65100" stroke-width="4"/>'
    svg += label("Пружина", 310, 140, size=11, color="#e65100")

    # Shock absorber body
    svg += '<rect x="240" y="55" width="20" height="220" rx="10" fill="#bbdefb" stroke="#1565c0" stroke-width="2"/>'
    svg += label("Амортизатор\n(газомасляный)", 270, 130, size=10, color="#1565c0")

    # Piston rod
    svg += '<rect x="247" y="30" width="6" height="30" fill="#90a4ae" stroke="#546e7a" stroke-width="1"/>'
    svg += label("Шток", 260, 45, size=9, color="#546e7a")

    # Knuckle / steering arm
    svg += '<rect x="230" y="270" width="40" height="50" rx="5" fill="#ffe0b2" stroke="#e65100" stroke-width="2"/>'
    svg += label("Поворотный\nкулак", 280, 285, size=10, color="#e65100")

    # Lower arm
    svg += '<path d="M 230 320 L 80 370 L 80 380" fill="none" stroke="#1565c0" stroke-width="6" stroke-linecap="round"/>'
    svg += label("Нижний рычаг", 130, 365, size=10, color="#1565c0")

    # Ball joint
    svg += '<circle cx="230" cy="320" r="8" fill="#c62828" stroke="#b71c1c" stroke-width="2"/>'
    svg += label("Шаровая\nопора", 200, 345, size=9, color="#c62828")

    # Stabilizer link
    svg += '<line x1="300" y1="250" x2="350" y2="200" stroke="#6a1b9a" stroke-width="4" stroke-linecap="round"/>'
    svg += label("Стойка\nстабилизатора", 360, 215, size=9, color="#6a1b9a")

    # Stabilizer bar
    svg += '<line x1="350" y1="200" x2="450" y2="180" stroke="#6a1b9a" stroke-width="6" stroke-linecap="round"/>'
    svg += label("Стабилизатор\nпоперечной\nустойчивости", 440, 175, size=9, color="#6a1b9a")

    # Drive shaft
    svg += '<line x1="100" y1="260" x2="250" y2="290" stroke="#333" stroke-width="5" stroke-linecap="round"/>'
    svg += label("Приводной вал\n(ШРУС)", 100, 250, size=9, color="#333")

    # Wheel hub
    svg += '<rect x="230" y="295" width="15" height="15" rx="3" fill="#666" stroke="#444" stroke-width="1"/>'
...
def gen_battery():
    ...
    # Battery body
    svg += '<rect x="200" y="60" width="200" height="160" rx="15" fill="#e8eaf6" stroke="#283593" stroke-width="3"/>'
    svg += '<rect x="210" y="70" width="180" height="30" rx="5" fill="#c5cae9" stroke="#283593" stroke-width="1"/>'

    # Terminals
    svg += '<rect x="255" y="40" width="10" height="25" rx="3" fill="#c62828"/>'
    svg += label("+", 260, 35, size=14, color="#c62828", bold=True)
    svg += label("Клемма (+)", 260, 25, size=10, color="#c62828")

    svg += '<rect x="335" y="40" width="10" height="25" rx="3" fill="#333"/>'
    ...
    # Electrolyte level
    svg += '<rect x="220" y="145" width="25" height="60" rx="2" fill="#bbdefb" opacity="0.5"/>'
    svg += label("Электролит\n(H₂SO₄)", 210, 170, size=9, color="#1565c0")

    # Plates
    svg += '<rect x="225" y="155" width="3" height="40" fill="#999"/>'
    svg += '<rect x="237" y="155" width="3" height="40" fill="#999"/>'
...
def gen_timing_belt():
    ...
    svg += '<circle cx="300" cy="310" r="45" fill="none" stroke="#333" stroke-width="5"/>'
    svg += '<circle cx="300" cy="310" r="15" fill="#666" stroke="#333" stroke-width="2"/>'
    svg += '<rect x="285" y="340" width="30" height="20" rx="3" fill="#78909c" stroke="#546e7a" stroke-width="1"/>'
    svg += label("Шкив\nколенвала", 300, 275, size=10, color="#333", bold=True)
    svg += label("(звёздочка)", 300, 290, size=9, color="#666")

    # Camshaft pulley (top, offset)
    svg += '<circle cx="300" cy="90" r="35" fill="none" stroke="#1565c0" stroke-width="5"/>'
    svg += '<circle cx="300" cy="90" r="12" fill="#1565c0" stroke="#0d47a1" stroke-width="2"/>'
    svg += label("Шкив\nраспредвала", 300, 50, size=10, color="#1565c0", bold=True)

    # Belt
    svg += '<path d="M 300 45 L 300 265" stroke="#e65100" stroke-width="4" stroke-dasharray="8,4" fill="none"/>'
    svg += label("Ремень ГРМ", 310, 160, size=10, color="#e65100", bold=True)

    # Tensioner
    svg += '<circle cx="380" cy="200" r="15" fill="none" stroke="#6a1b9a" stroke-width="3"/>'
    svg += '<circle cx="380" cy="200" r="5" fill="#6a1b9a"/>'
    svg += label("Натяжной\nролик", 400, 195, size=9, color="#6a1b9a")

    # Water pump pulley
    svg += '<circle cx="400" cy="310" r="20" fill="none" stroke="#2e7d32" stroke-width="3"/>'
    svg += label("Шкив\nпомпы", 430, 310, size=9, color="#2e7d32")

    # Belt path outline
    svg += '<path d="M 300 55 L 300 265 L 350 310 L 380 215 L 380 185 L 300 55" fill="none" stroke="#e65100" stroke-width="2" opacity="0.3"/>'

    # Timing marks
    svg += '<line x1="300" y1="90" x2="300" y2="55" stroke="#c62828" stroke-width="2"/>'
    svg += '<line x1="300" y1="310" x2="300" y2="265" stroke="#c62828" stroke-width="2"/>'
...
def gen_noise_isolation():
    ...
    # Zone 1: Floor
    svg += '<path d="M 130 280 L 440 280 L 420 200 L 150 200 Z" fill="#e3f2fd" stroke="#1565c0" stroke-width="2" opacity="0.6"/>'
    svg += label("Зона 1: Пол\n(вибро- + шумоизоляция)", 285, 240, size=10, color="#1565c0", bold=True)

    # Zone 2: Doors
    svg += '<rect x="100" y="170" width="20" height="100" rx="3" fill="#fff3e0" stroke="#e65100" stroke-width="2"/>'
    svg += '<rect x="480" y="180" width="20" height="90" rx="3" fill="#fff3e0" stroke="#e65100" stroke-width="2"/>'
    svg += label("Зона 2: Двери\n(вибро + герметизация)", 510, 175, size=9, color="#e65100")

    # Zone 3: Trunk
    svg += '<ellipse cx="490" cy="280" rx="35" ry="30" fill="#e8f5e9" stroke="#2e7d32" stroke-width="2" opacity="0.6"/>'
    svg += label("Зона 3: Багажник\n(арки + ниша)", 530, 280, size=9, color="#2e7d32")

    # Zone 4: Hood
    svg += '<ellipse cx="250" cy="140" rx="50" ry="15" fill="#fce4ec" stroke="#c62828" stroke-width="2" opacity="0.5"/>'
...
def gen_alternator():
    ...
    # Main body
    svg += '<rect x="150" y="80" width="250" height="150" rx="20" fill="#e8eaf6" stroke="#283593" stroke-width="3"/>'

    # Stator
    svg += '<circle cx="275" cy="155" r="50" fill="none" stroke="#1565c0" stroke-width="4"/>'
    svg += label("Статор\n(обмотка)", 275, 155, size=10, color="#1565c0", bold=True)

    # Rotor
    svg += '<circle cx="275" cy="155" r="20" fill="#e65100" stroke="#bf360c" stroke-width="2"/>'
    svg += label("Ротор\n(магнит)", 275, 120, size=9, color="#bf360c")

    # Pulley
    svg += '<circle cx="120" cy="155" r="25" fill="none" stroke="#666" stroke-width="4"/>'
    svg += '<circle cx="120" cy="155" r="8" fill="#666"/>'
    svg += label("Шкив\nпривода", 120, 130, size=9, color="#666")

    # Belt
    svg += '<line x1="145" y1="155" x2="200" y2="155" stroke="#e65100" stroke-width="3"/>'
    svg += label("Ремень", 170, 170, size=8, color="#e65100")

    # Regulator
    svg += '<rect x="350" y="90" width="40" height="50" rx="5" fill="#fff3e0" stroke="#e65100" stroke-width="2"/>'
    svg += label("Реле-\nрегулятор", 370, 85, size=9, color="#e65100")

    # Diode bridge
    svg += '<rect x="350" y="150" width="40" height="50" rx="5" fill="#fce4ec" stroke="#c62828" stroke-width="2"/>'
    svg += label("Диодный\nмост", 370, 148, size=9, color="#c62828")

    # Output terminal
    svg += '<rect x="380" y="180" width="40" height="10" rx="3" fill="#c62828"/>'
